...
export default function HomePage() {
  ...
  const [errorMessage, setErrorMessage] = useState<string | null>(null); // Yeni hata state'i

  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setIsSubmitting(true);
    setErrorMessage(null); // Her gönderimde eski hatayı temizle
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMessage("Something went wrong with our servers. Please try again.");
      }
    } catch (error) {
      console.error(error);
          ...
          <div className="p-10 md:p-16 rounded-[40px] bg-zinc-950 border border-zinc-800 shadow-2xl min-h-[500px] flex flex-col justify-center relative overflow-hidden group">
            ...
            {!isSuccess ? (
              <form onSubmit={handleSubmit} className="space-y-8 relative z-10" suppressHydrationWarning>
                ...
                {/* YENİ: Inline Hata Mesajı Kutusu */}
                <AnimatePresence>
                  {errorMessage && (
                    <motion.div 
                      ...
                      className="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm text-center font-medium backdrop-blur-md overflow-hidden"
                    >
                      {errorMessage}
                    </motion.div>
                  )}
