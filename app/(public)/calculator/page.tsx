...
export default function PriceCalculatorPage() {
    ...
    const [errorMessage, setErrorMessage] = useState<string | null>(null); // YENİ EKLENDİ (Yukarıdaki state'lerin arasına koy)

    const handleCalculateAndSend = async (e: React.FormEvent) => {
        e.preventDefault();
        setLoading(true);
        setErrorMessage(null); // Eski hatayı temizle
        ...
        try {
            ...
            if (response.ok) {
                setCurrentStep(7); // Sadece başarılıysa son ekrana geç
            ...
            setErrorMessage("Bağlantı hatası. Lütfen internetinizi kontrol edin.");
        } finally {
            setLoading(false);
        }
    };
                ...
                {currentStep === 6 && (
                    <div className="space-y-16 animate-in fade-in slide-in-from-bottom-8 duration-700">
                        ...
                        <form onSubmit={handleCalculateAndSend} className="max-w-md mx-auto space-y-8 p-10 bg-white border-2 border-zinc-100 rounded-[40px] shadow-2xl shadow-zinc-100">
                            ...
                            {errorMessage && (
                                <p className="text-red-500 text-xs font-bold uppercase tracking-widest text-center mb-4">{errorMessage}</p>
                            )}
                            <button 
