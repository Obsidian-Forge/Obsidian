...
// ============================================================================
// NOVATRUM ENTERPRISE PRICING & LOGIC ENGINE (PURE WHITE EDITION)
// ============================================================================

const SETUP_FEE = 300;

const ARCHITECTURE_OPTIONS = [
    { id: "landing", price: 1250, label: "Landing Architecture", desc: "High-conversion single page presence. Optimized for performance and ad campaigns." },
    { id: "corporate", price: 2500, label: "Corporate Platform", desc: "Multi-page professional business site with CMS capabilities and deep routing." },
    { id: "ecommerce", price: 4000, label: "Digital Storefront", desc: "Full e-commerce capabilities with custom inventory, cart flows, and tax logic." },
    { id: "saas", price: 6000, label: "Custom Software (SaaS)", desc: "Complex bespoke web applications, custom databases, and proprietary business logic." }
];

const DESIGN_STYLES = [
    { id: "minimal", price: 0, label: "Scandinavian Minimal", desc: "Extreme focus on whitespace, clean lines, and typography. Timeless." },
    { id: "bold", price: 800, label: "Bold & Editorial", desc: "High contrast, large typography, edgy and memorable. Magazine-like feel." },
    { id: "corporate", price: 0, label: "Modern Corporate", desc: "Trustworthy, highly structured, and data-forward aesthetic." },
    { id: "interactive", price: 4500, label: "Award-Winning Immersive", desc: "Heavy WebGL, scroll animations, 3D interactions, and fluid transitions." }
];

const INTEGRATION_OPTIONS = [
    { id: "stripe", price: 1500, label: "Payment Infrastructure", desc: "Secure checkout, subscriptions, and invoicing via Stripe/Mollie." },
    { id: "auth", price: 1800, label: "Authentication & Roles", desc: "Encrypted user login, JWT tokens, and multi-tier permission layers." },
    { id: "crm", price: 2000, label: "CRM Sync", desc: "Direct bidirectional sync with Hubspot, Salesforce, or custom endpoints." },
    { id: "mail", price: 600, label: "Transactional Email", desc: "Automated event-based email flows (SendGrid/Resend)." },
    { id: "analytics", price: 800, label: "Telemetry & Analytics", desc: "Granular user behavior, event tracking, and custom conversion funnels." },
    { id: "ai", price: 5000, label: "AI & LLM Pipelines", desc: "Custom AI workflows, automated assistants, and RAG architectures." },
    { id: "sockets", price: 3000, label: "Real-time WebSockets", desc: "Live data feeds, instant messaging, and collaborative interfaces." },
    { id: "cms", price: 3500, label: "Bespoke Admin Control", desc: "Custom-built headless CMS tailored exactly to your operational needs." },
    { id: "multilang", price: 1500, label: "Multi-language (i18n)", desc: "Full locale routing, dynamic translation engine, and SEO localization." }
];

const RESTRICTED_FOR_LANDING = ['stripe', 'auth', 'crm', 'sockets', 'cms', 'ai'];

const TIMELINES = [
    { id: "relaxed", multiplier: 0.9, label: "Relaxed Delivery", desc: "Extended delivery window. 10% operational discount applied." },
    { id: "standard", multiplier: 1.0, label: "Standard Pipeline", desc: "Regular agency queue priority. Expected delivery in 4-8 weeks." },
    { id: "expedited", multiplier: 1.5, label: "Expedited Priority", desc: "Fast-tracked delivery. Dedicated team allocation. 50% premium." },
...
const MAINTENANCE_OPTIONS = [
    { id: "none", price: 0, title: "No Ongoing Support", desc: "Client assumes full responsibility for server security, API changes, and updates post-launch." },
    { id: "essential", price: 299, title: "Essential Security Retainer", desc: "Proactive security patches, uptime monitoring, daily backups, and core infrastructure updates." },
    { id: "growth", price: 599, title: "Growth Engineering Retainer", desc: "Essential package + 5 hours of monthly continuous development and UI iterations." },
    { id: "scale", price: 1299, title: "Scale Dedicated Retainer", desc: "Essential package + 15 hours of development. Priority slack channel and dedicated tech lead." }
];
...
function DiscoveryFormContent() {
    ...
    const downloadClientCopy = async () => {
        ...
        doc.setFontSize(10); doc.setFont("helvetica", "normal"); doc.setTextColor(113, 113, 122); doc.text("Premium Software Studio", 20, 45);
        doc.setFontSize(20); doc.setFont("helvetica", "bold"); doc.setTextColor(24, 24, 27); doc.text("ARCHITECTURE BLUEPRINT", pageWidth - 20, 30, { align: "right" });
        doc.setFontSize(10); doc.setFont("helvetica", "normal"); doc.setTextColor(113, 113, 122); doc.text(`Ref ID: ${discoveryId}`, pageWidth - 20, 38, { align: "right" }); doc.text(`Date: ${new Date().toLocaleDateString()}`, pageWidth - 20, 44, { align: "right" });
        ...
        addRow("Entity Name:", formData.companyName); addRow("Current Web:", formData.currentWebsite || "None"); addRow("Primary Goal:", formData.projectGoals); addRow("Target Audience:", formData.targetAudience); addRow("Unique Value:", formData.uniqueValue); addRow("Competitors:", formData.competitors); yPos += 5;
        
        addSectionTitle("2. TECHNICAL ARCHITECTURE"); 
        const arch = ARCHITECTURE_OPTIONS.find(a => a.id === formData.architecture); addRow("Core Platform:", arch?.label || "Not set", arch ? `+€${arch.price.toLocaleString()}` : ""); 
        const extraPages = formData.estimatedPages > 5 && formData.architecture !== 'landing' ? (formData.estimatedPages - 5) * 200 : 0; addRow("Page Count:", `${formData.architecture === 'landing' ? '1' : formData.estimatedPages} Pages`, extraPages > 0 ? `+€${extraPages.toLocaleString()}` : "Included"); 
        addRow("Base Setup & Ops:", "Cloud Provisioning & Framework Init", `+€${SETUP_FEE}`); yPos += 5;

        addSectionTitle("3. VISUAL ENGINEERING"); 
        const design = DESIGN_STYLES.find(d => d.id === formData.designStyle); addRow("Design Style:", design?.label || "Not set", design && design.price > 0 ? `+€${design.price.toLocaleString()}` : "Included"); addRow("Brand Colors:", `Primary: ${formData.primaryColor}, Secondary: ${formData.secondaryColor}`, "Included"); addRow("Typography Base:", formData.fontPreference, "Included"); addRow("Copywriting:", formData.needsCopywriting ? "Novatrum SEO Copywriting" : "Client Provided", formData.needsCopywriting ? "+€1,500" : "Included"); yPos += 5;
        ...
        const time = TIMELINES.find(t => t.id === formData.timeline); let timeMod = "Included"; if (time && time.multiplier !== 1.0) { const base = calculateBaseTotal(); timeMod = `${time.multiplier > 1 ? '+' : '-'}€${Math.abs((base * time.multiplier) - base).toLocaleString()}`; } addRow("Delivery Timeline:", time?.label || "Standard", timeMod); 
        ...
        doc.setFontSize(14); doc.setFont("helvetica", "bold"); doc.text("TOTAL ESTIMATED INVESTMENT", 25, yPos); doc.setFont("helvetica", "bold"); doc.text(`€${calculateTotal().toLocaleString()}`, pageWidth - 25, yPos, { align: "right" }); 
        yPos += 6; doc.setFontSize(8); doc.setFont("helvetica", "italic"); doc.setTextColor(150, 150, 150); doc.text("* Estimate represents engineering value. Excludes VAT.", pageWidth - 25, yPos, { align: "right" }); doc.setTextColor(0, 0, 0); 
        ...
        const footerY = doc.internal.pageSize.getHeight() - 20; doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(150, 150, 150); doc.text("Definitive blueprint securely logged in the Novatrum internal network.", 20, footerY);
                    ...
                    <div className="max-w-7xl mx-auto flex items-center justify-between gap-6">
                        <div className="flex items-center gap-6 w-full md:w-auto">
                            ...
                            <div className="flex flex-col">
                                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">Discovery Protocol</span>
                                <span className="text-sm font-black tracking-widest uppercase">Phase 0{currentStep} / 0{totalSteps}</span>
                            </div>
                        </div>
                        
                        <div className="hidden md:flex flex-1 max-w-2xl mx-12 h-[2px] bg-zinc-100 overflow-hidden">
                            <div className="h-full bg-black transition-all duration-700 ease-in-out" style={{ width: `${(currentStep / totalSteps) * 100}%` }} />
                        </div>

                        <div className="hidden md:flex flex-col items-end">
                            <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-zinc-400 mb-1">Live Architectural Estimate</span>
            ...
            <div className="max-w-4xl mx-auto p-5 md:p-10 pt-10 md:pt-20">
                ...
                {currentStep === 1 && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                        <div>
                            <h1 className="text-5xl md:text-7xl font-light tracking-tighter text-black">Initiation.</h1>
                            <p className="text-zinc-400 font-medium mt-6 text-lg tracking-tight">Define the core entity and primary objective of the digital architecture.</p>
                        </div>

                        <div className="bg-white border border-zinc-200 p-8 rounded-3xl flex flex-col md:flex-row items-start md:items-center gap-6">
                            ...
                            <div>
                                <h3 className="text-xs font-bold uppercase tracking-widest text-black mb-2">Initialization Fee: €{SETUP_FEE}</h3>
                                <p className="text-sm font-medium text-zinc-500 leading-relaxed max-w-xl">Every Novatrum architecture begins with a standard operational setup. This mandatory fee covers pristine cloud provisioning, SSL/DNS configurations, and base framework scaffolding.</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-8">
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 pl-4">Corporate Entity / Brand Name *</label>
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 pl-4">Existing Domain (Optional)</label>
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 pl-4">Primary Objective *</label>
                                <textarea placeholder="What specific business problem are we solving? (e.g., Automating bookings, increasing B2B leads, launching a SaaS product)" value={formData.projectGoals} onChange={(e) => setFormData({...formData, projectGoals: e.target.value})} className="w-full bg-white border border-zinc-200 p-6 rounded-3xl outline-none text-base font-medium focus:border-black transition-colors h-40 resize-none" />
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                        ...
                        <div className="space-y-8">
                            <div className="space-y-3">
                                ...
                                <textarea placeholder="Who are the primary users? Describe their demographics and technical proficiency." value={formData.targetAudience} onChange={(e) => setFormData({...formData, targetAudience: e.target.value})} className="w-full bg-white border border-zinc-200 p-6 rounded-3xl outline-none text-base font-medium focus:border-black transition-colors h-32 resize-none" />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 pl-4">Unique Value Proposition</label>
                                <textarea placeholder="What makes your offering fundamentally better than the alternative?" value={formData.uniqueValue} onChange={(e) => setFormData({...formData, uniqueValue: e.target.value})} className="w-full bg-white border border-zinc-200 p-6 rounded-3xl outline-none text-base font-medium focus:border-black transition-colors h-32 resize-none" />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 pl-4">Direct Competitors / Benchmarks</label>
                                <textarea placeholder="List URLs of competitors or platforms whose quality you wish to match or exceed." value={formData.competitors} onChange={(e) => setFormData({...formData, competitors: e.target.value})} className="w-full bg-white border border-zinc-200 p-6 rounded-3xl outline-none text-base font-medium focus:border-black transition-colors h-24 resize-none" />
                            </div>
                        </div>
                    </div>
                )}
                ...
                {currentStep === 3 && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                         <div>
                            <h1 className="text-5xl md:text-7xl font-light tracking-tighter text-black">Architecture.</h1>
                            <p className="text-zinc-400 font-medium mt-6 text-lg tracking-tight">Select the foundational technical structure for your deployment.</p>
                         </div>
...
onClick={() => setFormData(prev => ({ ...prev, architecture: opt.id, estimatedPages: opt.id === 'landing' ? 1 : (prev.architecture === 'landing' ? 5 : prev.estimatedPages) }))}                                        className={`p-8 rounded-[32px] border transition-all duration-300 cursor-pointer flex flex-col justify-between ${isSelected ? 'border-black shadow-md bg-white scale-[1.02]' : 'border-zinc-200 hover:border-zinc-400 bg-white'}`}
                         ...
                         <div className={`bg-white p-8 md:p-10 rounded-[32px] border border-zinc-200 transition-opacity duration-500 ${formData.architecture === 'landing' ? 'opacity-30 pointer-events-none grayscale' : ''}`}>
                            <div className="flex justify-between items-end mb-8">
                                <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Scale Requirement (Page Count)</label>
                ...
                {currentStep === 4 && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                        <div>
                            <h1 className="text-5xl md:text-7xl font-light tracking-tighter text-black">Aesthetics.</h1>
                            <p className="text-zinc-400 font-medium mt-6 text-lg tracking-tight">Define the visual engineering and brand parameters.</p>
                        </div>
                        ...
                        <div className="bg-white p-8 md:p-10 rounded-[32px] border border-zinc-200 grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div className="space-y-6">
                                ...
                                <div className="flex gap-4">
                                    <div className="flex-1 bg-white p-4 rounded-2xl border border-zinc-200 flex flex-col items-center gap-4">
                                        <span className="text-[9px] font-bold uppercase text-zinc-400 tracking-widest">Primary</span>
                                    ...
                                    {formData.hasAccentColor ? (
                                        <div className="flex-1 bg-white p-4 rounded-2xl border border-zinc-200 flex flex-col items-center gap-4 relative">
                                            ...
                                            <span className="text-[9px] font-bold uppercase text-zinc-400 tracking-widest">Accent</span>
                                        ...
                                        <button onClick={() => setFormData({...formData, hasAccentColor: true})} className="flex-1 bg-white border-2 border-dashed border-zinc-200 rounded-2xl flex flex-col items-center justify-center gap-2 hover:border-black transition-all">
                                            ...
                                            <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-400">Add Accent</span>
                                        </button>
                                    )}
                                 </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 pl-2">Typography Base</label>
                ...
                {currentStep === 5 && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                        <div>
                            <h1 className="text-5xl md:text-7xl font-light tracking-tighter text-black">Assets.</h1>
                        ...
                        <div className={`p-8 rounded-[32px] border transition-all cursor-pointer flex flex-col md:flex-row items-start md:items-center justify-between gap-6 ${formData.needsCopywriting ? 'border-black bg-black text-white shadow-md scale-[1.02]' : 'border-zinc-200 hover:border-black bg-white text-black'}`} onClick={() => setFormData({...formData, needsCopywriting: !formData.needsCopywriting})}>
                            <div className="flex gap-6 items-center">
                                ...
                                <div>
                                    <h3 className="text-2xl font-light tracking-tight">Novatrum SEO Copywriting</h3>
                        ...
                        <div className="bg-white p-8 md:p-12 rounded-[32px] border border-zinc-200 space-y-6">
                            <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 pl-2">Asset Upload Center</label>
                            ...
                            <div className="border-2 border-dashed border-zinc-200 rounded-3xl p-12 text-center bg-white hover:border-black transition-all relative cursor-pointer group" onClick={() => fileInputRef.current?.click()}>
                                ...
                                <div className="space-y-4 pointer-events-none">
                                    {uploading ? (
                                     ...
                                     <p className="text-base font-bold tracking-tight text-black">{uploading ? 'Transmitting Data Protocol...' : 'Click to Upload Materials'}</p>
                ...
                {currentStep === 6 && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                        <div>
                            <h1 className="text-5xl md:text-7xl font-light tracking-tighter text-black">Matrix.</h1>
                            <p className="text-zinc-400 font-medium mt-6 text-lg tracking-tight">Select backend capabilities and API integrations.</p>
                        </div>
                ...
                {currentStep === 8 && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                        <div>
                            <h1 className="text-5xl md:text-7xl font-light tracking-tighter text-black">Logistics.</h1>
                            <p className="text-zinc-400 font-medium mt-6 text-lg tracking-tight">Define delivery timeline and ongoing engineering retainer.</p>
                        </div>
                        ...
                        <div className="space-y-8">
                             {isComplexProject && (
                                <div className="bg-white p-6 rounded-3xl flex items-start gap-6 border border-zinc-200">
                                    ...
                                    <div>
                                        <p className="text-xs font-bold uppercase tracking-widest text-black mb-1">Scale Constraint Enforced</p>
                                        <p className="text-sm font-medium text-zinc-500">Due to the selected architecture complexity, standard timelines are disabled. Enterprise scale timeline is mandatory for quality assurance.</p>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="bg-white p-8 md:p-10 rounded-[32px] border border-zinc-200 space-y-6">
                                <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 block border-b border-zinc-100 pb-4">Continuous Engineering (Retainer)</label>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex flex-col items-end shrink-0 pl-9 md:pl-0">
                                                    ...
                                                    {opt.price > 0 && isSelected && <span className="text-[9px] font-bold text-black uppercase mt-2 tracking-widest">2 Months Free</span>}
                                                </div>
                                            </div>
                                        );
                                    })}
                ...
                {currentStep === 9 && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                        <div>
                            <h1 className="text-5xl md:text-7xl font-light tracking-tighter text-black">Authorization.</h1>
                            <p className="text-zinc-400 font-medium mt-6 text-lg tracking-tight">Finalize billing credentials to generate the official blueprint.</p>
                        </div>

                        <div className="bg-white border border-zinc-200 p-8 md:p-12 rounded-[40px] space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase text-zinc-400 tracking-widest pl-2">Authorized Signatory *</label>
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase text-zinc-400 tracking-widest pl-2">Corporate Email *</label>
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase text-zinc-400 tracking-widest pl-2">Phone Number</label>
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase text-zinc-400 tracking-widest pl-2">VAT Number</label>
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-bold uppercase text-zinc-400 tracking-widest pl-2">Billing Address *</label>
                            ...
                            <div className="mt-8 p-6 bg-white border border-zinc-200 rounded-3xl flex items-start gap-5 hover:border-black transition-colors cursor-pointer" onClick={() => setFormData({...formData, acceptedTerms: !formData.acceptedTerms})}>
                                ...
                                <div className="text-sm font-medium text-zinc-600 leading-relaxed flex-1 select-none">
                                    I authorize Novatrum to generate a definitive architecture blueprint. I understand this represents engineering value. Final contracting will include this value plus operational fees (incl. the €{SETUP_FEE} initialization fee) and applicable VAT. I accept the <a href="/legal/terms" target="_blank" onClick={(e) => e.stopPropagation()} className="text-black font-bold underline hover:text-zinc-500 transition-colors">Master Service Agreement</a>.
                                </div>
                             </div>
                        </div>
                    </div>
                ...
                {currentStep === 10 && (
                     <div className="text-center space-y-12 animate-in zoom-in-95 py-16">
                        ...
                        <div>
                            <h2 className="text-5xl md:text-7xl font-light tracking-tighter text-black">Protocol Logged.</h2>
                            <p className="text-zinc-500 font-medium max-w-lg mx-auto mt-6 text-lg">
                                Architecture specifications secured. Engineering review pending.
                            </p>
                        </div>
                         ...
                         <div className="bg-white p-10 md:p-12 border border-zinc-200 rounded-[40px] max-w-md mx-auto text-left relative overflow-hidden">
                            ...
                            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-2">Deployment Reference</p>
                        ...
                        <div className="mt-12 flex flex-col items-center gap-4">
                            <button onClick={() => router.push('/')} className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 hover:text-black transition-colors underline underline-offset-8">Return to Central Gateway</button>
                        </div>
                    </div>
                )}
            </div>
            ...
            {currentStep < 10 && (
                 <div className="fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-xl border-t border-zinc-200 p-5 md:p-8 z-40">
                    <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
                        ...
                        <div className="flex flex-col">
                             <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-1 hidden md:block">Total Investment Summary</p>
                            <div className="flex items-end gap-3">
                            ...
                            <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-400 mt-1">Excl. VAT</p>
                        </div>
                            ...
                            {currentStep === 9 ? (
                                <button onClick={submitDiscovery} disabled={loading || !isStepValid()} className="bg-black text-white px-8 md:px-16 py-4 md:py-5 rounded-full font-bold uppercase text-[10px] md:text-xs tracking-[0.2em] hover:scale-105 active:scale-95 transition-all shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-3">
                                 ...
                                 <button onClick={nextStep} disabled={!isStepValid()} className="bg-black text-white px-8 md:px-16 py-4 md:py-5 rounded-full font-bold uppercase text-[10px] md:text-xs tracking-[0.2em] hover:scale-105 active:scale-95 transition-all shadow-md disabled:opacity-30 disabled:cursor-not-allowed flex items-center gap-3">
                                     Next Phase <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg>
                                </button>
                            )}
                         </div>
                    </div>
